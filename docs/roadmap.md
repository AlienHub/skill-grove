...
> 当前目标：把 MVP 从“能扫描、能展示、能管理来源”的工具，打磨成一个精致、安静、本地优先的 Agent Skills Library。
>
...
- [ ] 将主页面语义从「所有技能」调整为「技能库 / Library」
- [ ] 详情页改成 **Variant-first** 信息结构
- [ ] 来源区域从「按 Source 展示」调整为「按内容版本 Variant 展示」
- [ ] 来源 dropdown / popover 改为按 Variant 分组
- [ ] 对 20+ 来源场景做聚合展示：默认只展示版本摘要，不平铺所有来源
- [ ] 在 skill 详情页添加统一操作入口：打开目录、用 IDE/编辑器打开、复制路径、移除此来源
- [ ] 增加安全删除能力：只允许删除单个 Source，并优先移动到系统废纸篓
- [ ] 对 symlink 删除做单独文案：只移除软链接，不删除目标目录
- [ ] 弱化重复的「真实文件」标签，只在软链接、异常、缺失等非默认状态时突出显示
- [ ] 调整文案：减少“管理 / 警告 / 错误”，增加“来源 / 内容版本 / 值得看看 / 保持一致”

### v0.2.0a：Variant-first reading

- [ ] 左侧「所有技能」改为「技能库」
- [ ] 详情页顶部突出「N 个内容版本 · M 个来源」
- [ ] 来源模块按 Variant 摘要展示
- [ ] 来源 dropdown / popover 按 Variant 分组
- [ ] 弱化「真实文件」，只突出软链接/异常

### v0.2.0b：Source actions

- [x] 增加 Source 操作菜单：打开目录、用编辑器打开、复制路径、显示 SKILL.md
- [x] 增加「移除此来源」确认弹窗
...
- [ ] 将左侧标题改为「技能库」
- [ ] 将详情页顶部的「N 个内容版本 · M 个来源」提升为核心状态
- [ ] 将来源模块改成按 Variant 展示
- [ ] 将来源 dropdown 改成按 Variant 分组
- [ ] 将「选择来源」改为「查看来源」或「切换来源」
- [ ] 弱化「真实文件」标签，只突出软链接、异常、缺失等非默认状态

v0.2.0b 再处理：

- [x] 增加 Source 操作菜单：打开目录、用编辑器打开、复制路径、移除此来源
- [x] 实现「移除此来源」到系统废纸篓
- [x] 删除后自动重新扫描并刷新详情

完成 v0.2.0a 后，产品调性会明显从「skill 文件管理器」升级为「本地 Agent Skills Library」。
...
> 让用户一眼看懂：这个 skill 有几个内容版本，分布在哪些 agent 里，当前看到的是哪一版。
