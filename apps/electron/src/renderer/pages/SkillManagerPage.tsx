...
function SkillDirectoryConfig({
  ...
  return (
    <div className="space-y-6">
      <section>
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[12px] text-foreground/52">管理需要扫描的 skill 根目录</p>
          </div>
          <button
            className="cursor-pointer text-[12px] text-foreground/52 transition-colors hover:text-foreground"
            onClick={onRefresh}
            type="button"
          >
            刷新
      ...
      <section>
        <div className="mb-4">
          <h3 className="text-[14px] font-semibold text-foreground">添加目录</h3>
          <p className="mt-1 text-[12px] text-foreground/52">支持手动输入，也支持选择文件夹</p>
        </div>
        ...
        <div className="space-y-3">
          <form className="flex gap-2" onSubmit={handleSubmit}>
            <input
              className="min-w-0 flex-1 rounded-[8px] border border-border/50 bg-white px-3 py-2 text-[12px] text-foreground outline-none placeholder:text-foreground/35"
              disabled={inputDisabled}
              onChange={(event) => onInputChange(event.target.value)}
              placeholder="输入 skill 目录，例如 ~/my-skills"
              value={inputValue}
            />
            <button
        ...
        {feedbackMessage ? (
          <p className="mt-3 text-[12px] text-foreground/52">{feedbackMessage}</p>
        ) : null}
      </section>

      <section>
        <div className="mb-4">
          <h3 className="text-[14px] font-semibold text-foreground">发现到的目录</h3>
          <p className="mt-1 text-[12px] text-foreground/52">当前扫描结果里实际命中的目录</p>
        </div>
        ...
        {discoveredDirectories.length === 0 ? (
          ...
          <div className="space-y-2">
            {discoveredDirectories.map((directory) => (
              <div
                className="truncate rounded-[8px] border border-border/50 bg-white px-4 py-3 text-[12px] text-foreground/68 shadow-minimal-flat"
                key={directory}
                title={directory}
              >
                {directory}
...
export function SkillManagerPage() {
  ...
  const handleAddDirectory = async () => {
    ...
    await updateDirectories([...skillState.configuredDirectories, nextDirectory])
    setDirectoryInput('')
    setDirectoryFeedbackMessage(null)
  }
  ...
  const handleChooseDirectory = async (directory: string) => {
    if (!directory) {
      setDirectoryFeedbackMessage('当前环境没有返回真实文件夹路径，请改用手动输入。')
      return
    }
  ...
  return (
    <div className="h-screen overflow-hidden bg-background text-[14px] text-foreground">
      <main className="mx-auto h-full max-w-[1440px] px-4 py-6 sm:px-6">
              ...
              <div className="mb-8 rounded-[8px]">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-[14px] leading-7 text-foreground/56">
                    在这里配置需要扫描的 skill 目录，并查看当前实际发现到的目录。
                  </p>
                </div>
              </div>
